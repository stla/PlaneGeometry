...
#' @title Maximum area circle inscribed in a convex polygon
#' @description Computes the circle inscribed in a convex polygon with
...
#'
#' @return A \code{Circle} object. The status of the optimization problem
#'   is given as an attribute of this circle. A warning is thrown if it is
#'   not optimal.
#' @export
#' @seealso \code{\link{maxAreaInscribedEllipse}}
#' @importFrom rcdd makeV scdd
#' @importFrom CVXR Variable Maximize norm2 Problem psolve
#' @examples
...
#' attr(circ, "status")
maxAreaInscribedCircle <- function(points, verbose = FALSE) {
